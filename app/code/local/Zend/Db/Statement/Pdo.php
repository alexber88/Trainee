...
class Zend_Db_Statement_Pdo extends Zend_Db_Statement implements IteratorAggregate
{
    ...
    {
        //cgi code start: deadlock fix
        if (Mage::registry('dev/deadlock/enabled')) {
            $errors = array(
                0 => 'SQLSTATE[40001]: Serialization failure: 1213 Deadlock found when trying to get lock; try restarting transaction',
                1 => 'SQLSTATE[23000]: Integrity constraint violation: 1452 Cannot add or update a child row: a foreign key constraint fails'
            );
            ...
            do {
                ...
                try {
                    ...
                    if ($tries < 3 && ($message == $errors[0] || strstr($message, $errors[1]))) {
                        $retry = true;
                    } else {
                        throw new Zend_Db_Statement_Exception($exception->getMessage());
                    }
                    if ($tries) {
                        sleep(((int) pow($tries, 2)) -1);
                    }
                    $tries++;
                }
            } while ($retry);
